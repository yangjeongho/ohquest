(function ($) {
	...
	function Monthpicker() {
		...
		this._unselectableClass = 'ui-datepicker-unselectable'; //max
	...
	$.extend(Monthpicker.prototype, {
		...
		_optionMonthpicker: function(target, name, value) {
			...
			if (inst) {
				...
				var date = this._getDateMonthpicker(target, true);
...
//			console.log("_adjustInstDate");
//			console.log(inst);
//			console.log(inst.drawYear);
//			
...
//			}
			...
			for(var i=0; i<=11; i++){
				...
				currentDate= new Date(inst.selectedYear, i, 1);
				...
				html += '<td style="padding:1px;cursor:default;" data-month=' + i + '>'
					...
					+ (maxDate!= null && (currentDate > maxDate) || (currentDate < minDate) ? ' '+ this._unselectableClass +' ui-state-disabled' : '') //추가
					+ '"'+ (maxDate!= null &&  (currentDate > maxDate) || (currentDate < minDate) ? '' : ' onclick="MP_jQuery_' + dpuuid 
							+ '.monthpicker._selectMonth(\'#'+ inst.id + '\',' + drawYear + ', ' + i + ');return false;"') 
					+ 'actionshref="#">' + (inst.settings && inst.settings.monthNamesShort ? inst.settings.monthNamesShort[i] : this._defaults.monthNamesShort[i]) + '</a>' + '</td>'; // display selectable date
...
//			console.log("_daylightSavingAdjust");
//			console.log(date);
...
//			console.log(inst.currentMonth);
		...
		return dpDiv.delegate(selector, 'mouseout', function() {
			...
			.delegate(selector, 'mouseover', function(){
				if (!$.monthpicker._isDisabledMonthpicker( instActive.input[0])) {
					...
					//추가
					$(this).not(".ui-datepicker-unselectable, .ui-state-disabled").addClass('ui-state-hover');
					//original 
	...
	$.fn.monthpicker = function(options){
		...
		if (typeof options == 'string' && (options == 'isDisabled' || options == 'getDate' || options == 'widget'))
			return $.datepicker['_' + options + 'Monthpicker'].
