...
function coverageSearchGridInit(form,  table){
...
// 	$('#searchAddressForm #searchKey').val('${searchFilter.searchKey[0]}');
	...
	coverageSearchGridInit("coverageSearchForm", "coverageSearchDivListGridTable");
	
	$("#coverageSearchForm #searchValue").keydown(function(e){
...
function initAddr(){
	...
	//조회
	$("#coverageSearchForm #searchAddressForm").click(function(e){
		
		e.preventDefault();
		
/* 		if($("#coverageSearchForm #searchValue").val() ==''){
			$("#coverageSearchForm #searchValue").focus();
			return;
		}
*/
		var cityCode = $("#coverageSearchForm #cityCode").val();
		var districtCode = $("#coverageSearchForm #districtCode").val();
		var subDistrictCode = $("#coverageSearchForm #subDistrictCode").val();
		
		$("#coverageSearchForm #searchValue11").hide();
		$("#coverageSearchForm #searchValue22").hide();
		$("#coverageSearchForm #searchValue33").hide();
		$("#coverageSearchForm #searchValue44").hide();
		$("#coverageSearchForm #searchValue33").hide();
		$("#coverageSearchForm #searchValue44").hide();
		...
		if(cityCode == "" || cityCode.length <= 0 || cityCode=='X'){
			...
			$(this).loadingDialog();
			coverageSearchGridInit("coverageSearchForm", "coverageSearchDivListGridTable");
			coverageSearchGridAjaxCall(baseUrl + "common/address/houseHoldsearchList","coverageSearchForm", "coverageSearchDivListGridTable");
			$(this).hideLoading();
...
// 			$("#searchValue4").val("");
	...
	//네비
	$("#coverageSearchDivListPageNav").click(function(e){
		e.preventDefault();
		if(coverageSearchGridPageNavCom("coverageSearchForm") != false){
			$(this).loadingDialog();
			coverageSearchGridAjaxCall(baseUrl + "common/address/houseHoldsearchList","coverageSearchForm", "coverageSearchDivListGridTable");
			$(this).hideLoading();			
		};
	...
	//닫기
	$("#coverageSearchDiv01 #coverageSearchDivListCloseBtn").click(function(e){
		e.preventDefault();
		var jsonData = {
				addressFullName : "",
				addNum: "",
		...
		if(eval("window.${param.callBackFunc}") != null){
	...
	//확인
	$("#coverageSearchDiv01 #coverageSearchDivListConfirmBtn").click(function(e){
		if($("#coverageSearchDiv01 #subAddress").val() == ''){
			MessageBox.dialog('<s:message code="address.msg.000004"/>');
			return;
		}
		var jsonData = {
				addressFullName : $("#coverageSearchDiv01 #addressFullName").val(),
				addNum: $("#coverageSearchDiv01 #addNum").val(),
				doorCode : $("#coverageSearchDiv01 #coverageDoorCode").val(),
				subAddress: $("#coverageSearchDiv01 #subAddress").val(),					
				address: $("#coverageSearchDiv01 #addressFullName").val(),					
		};
		/* var jsonData = {
				addressFullName : $("#coverageSearchDiv01 #addressFullName").val(),
				addNum: $("#coverageSearchDiv01 #addNum").val(),
		...
		if(cityName != ""){	   
			if(event.keyCode > 40){
				...
				{	
			      	...
			      	source: 
			      		function () {
				      	  var data =  $("#coverageSearchForm").serializeArray();		      	
				    	  $.ajax({
				    		  type : "POST",  
				    		  async : true,
...
function coverageSearchGridPageNavCom(form){
...
function coverageSearchGridAjaxCall(curl, form, table){
...
//상품
$("#coverageSearchDivListGridTable").jqGrid({
	datatype : "local",
	height : 200,
	width : 590,
	colNames : [ 
    ...
    colModel : [ 
       	...
       	onSelectRow: function (id, status) {
       		var rowData = $('#coverageSearchDivListGridTable').getRowData(id); 
//        		autoBinding("#coverageDetailForm", rowData);
			...
			$("#coverageDetailForm #addNum").val(rowData.addressCode);
			$("#coverageDetailForm #addressFullName").val(address);
			$("#coverageDetailForm #coverageDoorCode").val(rowData.doorCode);
			
       		$("#coverageDetailForm #detailSearch").show();
       		$("#coverageDetailForm #subAddress").focus();
		}
});

function coverageSearchDivListResult(data){
	if(data.resultCode == "SUCCESS"){
		$("#coverageSearchDivListCloseBtn").click();
	}
}

</script>

<div id="coverageSearchDiv01">
	<div class="contents_box">
		<form id="coverageSearchForm" method="post">
		<input type="hidden" id="page" name="page" value="0" /> 
		<input type="hidden" id="callBackFunc" name="callBackFunc" value="0" /> 
		<input  id="servicedYn" name="servicedYn" type="hidden" value="Y"/>
			...
			<div class="contents_box f_left mr_20" style="width: 100%; ">
				...
				<div class="title">
					<ul class="btn_listtop f_right">
						...
						<li><a href="#" id="coverageSearchDivListPageNav"><img src="<s:url value="/resources/images/btn/btn_page.gif"/>" /></a></li>
					</ul>
				</div>			
				<div class="contents_box mt_5">
					<table id="coverageSearchDivListGridTable"></table>
				</div>
			</div>
		</form>
	</div>
	<br/>
	<form id="coverageDetailForm">
		...
		<table width="100%" cellpadding="0" cellspacing="0" border="1" class="table_view01">
			...
			<tr>
				...
				<td><input type="text" class="form_disable" style="width: 100%;" id="addNum" name="addNum"  title='<s:message code="ccs.csCccs000.taskHistoryListTable.lable.addNum"/>' readonly />
				<input type="hidden"  id="coverageDoorCode" name="coverageDoorCode"  /></td>
			</tr>
			<tr>
		...
		<table width="100%" cellpadding="0" cellspacing="0" border="1" class="table_view01 mt_5">
			...
			<tr>
				...
				<td><input type="text" class="form_normal01" style="width: 100%" id="subAddress" name="subAddress" title='<s:message code="prm.msg.000047"/>' /></td>
			</tr>
			</c:if>
			
		</table>
		<c:if test="${empty mode}">	
			<input type="hidden" class="form_normal01" style="width: 95%" id="subAddress" name="subAddress" title='<s:message code="prm.msg.000047"/>' />
		</c:if>
	</div>
   </form>
	<!-- 컨텐츠를 감싸는 박스 // -->
	<div class="contents_box mt_5 both">
		<!-- 버튼 //-->
		<div class="f_right">
			<span class="btns9 save"><input type="button" value= '<s:message code="cmn.btn.confirm"/>'  id="coverageSearchDivListConfirmBtn" /></span>
			<span class="btns7 save"><input type="button" value= '<s:message code="cmn.btn.close"/>'  id="coverageSearchDivListCloseBtn" /></span>
		</div>
		<!--// 버튼 -->
	</div>
