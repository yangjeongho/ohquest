...
(function($) {
    ...
    function buildFixedTable(src, section, target) {
        if ($(section, src).length) {
            ...
            //bsyang , col의 속성을 새로 생성되는 td에 반영하기 위함.
            var columnAttrArray = new Array();
            $(src).find("colgroup > col").each(function(e) {
            	var attrStr = "" ;
            	...
            	columnAttrArray[e] = attrStr;  
            });
            $(section + " tr", src).each(function() {
                var tr = $("<tr></tr>").appendTo(th);
                $(cellType, this).each(function(e) {
                	// th의 width를 반영함. bsyang
                	var attr = "" ;
                	if(columnAttrArray!=null){
                		attr="width=\""+columnAttrArray[e]+"\""; 
                	}
                    $("<td "+attr+">" + $(this).html() + "</td>").appendTo(tr);
                });
            });
    ...
    function adjustSizes(options) {
        ...
        $("#" + Id + " .fixedContainer ." + options.classHeader + " table tr:first td").each(function(pos) {
        	...
        	///alert("cwidth:"+cwidth +", width:" + $(this).attr('width'));
...
//alert(totall);
        ...
        /*
        $("#" + Id + " .fixedContainer ." + options.classHeader + " table").width("100%");
        $("#" + Id + " .fixedContainer .fixedTable table").width("100%");
        $("#" + Id + " .fixedContainer ." + options.classFooter + " table").width("100%"); 
		*/
